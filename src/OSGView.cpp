...
void osgView::modelplusx()
{
	osg::Vec3d vec=tractor->asPositionAttitudeTransform()->getPosition();
	vec.set(vec.x()+TRANS_STEP,vec.y(),vec.z());
	tractor->asPositionAttitudeTransform()->setPosition(vec);
}
void osgView::modelminusx()
{
	osg::Vec3d vec=tractor->asPositionAttitudeTransform()->getPosition();
	vec.set(vec.x()-TRANS_STEP,vec.y(),vec.z());
	tractor->asPositionAttitudeTransform()->setPosition(vec);
}
void osgView::modelplusy()
{
	osg::Vec3d vec=tractor->asPositionAttitudeTransform()->getPosition();
	vec.set(vec.x(),vec.y()+TRANS_STEP,vec.z());
	tractor->asPositionAttitudeTransform()->setPosition(vec);
}
void osgView::modelminusy()
{
	osg::Vec3d vec=tractor->asPositionAttitudeTransform()->getPosition();
	vec.set(vec.x(),vec.y()-TRANS_STEP,vec.z());
	tractor->asPositionAttitudeTransform()->setPosition(vec);
}
void osgView::modelplusz()
{
	osg::Vec3d vec=tractor->asPositionAttitudeTransform()->getPosition();
	vec.set(vec.x(),vec.y(),vec.z()+TRANS_STEP);
	tractor->asPositionAttitudeTransform()->setPosition(vec);
}
void osgView::modelminusz()
{
	osg::Vec3d vec=tractor->asPositionAttitudeTransform()->getPosition();
	vec.set(vec.x(),vec.y(),vec.z()-TRANS_STEP);
	tractor->asPositionAttitudeTransform()->setPosition(vec);
